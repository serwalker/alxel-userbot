...
async def amireallyalive(alive):
    ...
    output = (
        f"**Tʜᴇ [ALxEl-USERBOT](https://github.com/AyiinXd/Ayiin-Userbot)**\n\n"
        f"**{alive_text}**\n\n"
        f"╭✠╼━━━━━━━━━━━━━━━✠╮\n"
        f"{emoji} **Bᴀsᴇ Oɴ :** ••{adB.name}••\n"
        f"{emoji} **OWNER :** [{user.first_name}](tg://user?id={user.id}) \n"
        f"{emoji} **MODULS :** `{len(modules)} Modules` \n"
        f"{emoji} **BOT VERSION :** `{BOT_VER}` \n"
        f"{emoji} **PHYTON VERSION :** `{python_version()}` \n"
        f"{emoji} **PYTGCALS VERSION :** `{pytgcalls.__version__}` \n"
